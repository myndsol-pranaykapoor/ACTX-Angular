...
import { WebLoginScreenComponent } from './web-login-screen/web-login-screen.component';
...
export class App implements AfterViewInit, OnDestroy {
  ...
  protected closeUpdatesForm(): void {
    this.showUpdatesPopup.set(false);
  }
  ...
  constructor() {
    // Clear theme from localStorage to ensure login screen shows ACT theme
    try {
      localStorage.removeItem(this.THEME_STORAGE_KEY);
    } catch (e) {
      console.warn('Failed to clear theme from localStorage:', e);
    }
    // Ensure ACT theme is set as default for login screen
    this.themeService.setTheme('act');
  }
  ...
  private updateThemeMenuPosition(): void {
    ...
    // Get exact position of "Change Theme" option (2nd option in user menu)
    const options = userMenuOverlay.querySelectorAll('pds-option');
    if (options.length > 1) {
      const changeThemeOption = options[1];
      const optionRect = changeThemeOption.getBoundingClientRect();
      this.themeMenuTop = optionRect.top;
    } else {
      // Fallback if option not found
      this.themeMenuTop = rect.top + 44;
    }
