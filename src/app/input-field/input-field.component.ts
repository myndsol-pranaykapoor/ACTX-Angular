...
export class InputFieldComponent implements OnDestroy {
  ...
  /** Custom trailing icon — overrides the default type-based icon */
  @Input() customTrailingIcon = '';

  /** Show password as hidden (type="password") instead of visible (type="text") */
  @Input() showHiddenPassword = false;
  ...
  get hasMessage(): boolean {
    return this.showSupportingMessage || this.validationMessage !== null || this.effectiveState === 'error';
  }
  ...
  get trailingIcon(): string {
    // Use custom trailing icon if provided
    if (this.customTrailingIcon) return this.customTrailingIcon;
